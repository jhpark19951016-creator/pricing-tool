...
def infer_lawd_from_latlon(lawd_df: pd.DataFrame, lat: float, lon: float) -> Tuple[Optional[str], str]:
    ...
    - Nominatim reverse-geocode -> 시/도 + 시/군/구 추출
    - 우리 LAWD 목록(label)에서 부분일치로 매칭
    """
    ...
    # 예: '중구'만 있는 경우도 있음(동명 중복 주의)
    if dist_ko:
        candidates.append(dist_ko)

    labels = lawd_df["label"].astype(str)

    # 1) 후보 문자열로 우선 매칭(가장 빠름)
    for c in candidates:
        # regex=False 로 안전하게(특수문자/escape 문제 방지)
        hit = lawd_df[labels.str.contains(c, na=False, regex=False)]
        if not hit.empty:
            row = hit.iloc[0]
            ...
            if code.isdigit():
                code = code.zfill(10)
            return code, str(row.get("label", ""))

    # 2) fallback: display_name 기반으로 '시/도 시/군/구' 유사 매칭
    disp = str(data.get("display_name", ""))

    if metro_ko and dist_ko:
        key = f"{metro_ko} {dist_ko}"
        hit = lawd_df[labels.str.contains(key, na=False, regex=False)]
        if not hit.empty:
            row = hit.iloc[0]
            code = str(row.get("code", "")).strip()
            if code.isdigit():
    ...
    # 3) 마지막 fallback: label 전체 스캔(느림) -> 상위 2000개까지만(안정성)
    #    (리 단위까지 모두 넣었을 때도 앱이 멈추지 않게 제한)
    scan_n = min(len(lawd_df), 2000)
    for i in range(scan_n):
        lab = str(lawd_df.iloc[i].get("label", ""))
    ...
    return None, f"매칭 실패: {metro_ko or ''} {dist_ko or ''}".strip()
...
with left:
    ...
    out = st_folium(m, height=420, use_container_width=True) or {}
