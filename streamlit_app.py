...
with st.sidebar:
    ...
    # 기준 계약년월: 연/월 선택 (직접 입력 X)
    y_options = list(range(dt.date.today().year, 2005, -1))  # 2006~현재
    m_options = list(range(1, 13))
    col_y, col_m = st.columns([2, 1])
    with col_y:
        end_year = st.selectbox("기준 연도", options=y_options, index=0, key="s_end_year")
    with col_m:
        end_month = st.selectbox("기준 월", options=m_options, index=dt.date.today().month - 1, key="s_end_month")
    end_ym = f"{end_year}{end_month:02d}"

    # 최근 기간(개월): 선택형
    months = st.selectbox("최근 기간(개월)", options=list(range(1, 37)), index=11, key="s_months")
